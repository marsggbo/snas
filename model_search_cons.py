    ...
    def __init__(self, C, stride):
        ...
        for primitive in PRIMITIVES:
            ...
            if 'sep' in primitive:
                FLOP = 0
                MAC = 0
                FLOP1 = (self.height * self.width *
                         op.op[1].kernel_size[0]**2 * op.op[1].in_channels * op.op[1].out_channels)
                ...
                MAC2 = MAC1 + (op.op[1].in_channels *
                               op.op[1].out_channels) / op.op[1].groups
                MAC = (MAC1 + MAC2)*2

            if 'dil' in primitive:
                FLOP = (self.width*self.height*op.op[1].in_channels *
                        op.op[1].out_channels*op.op[1].kernel_size[0]**2) / op.op[1].groups
                FLOP += (self.width*self.height *
                         op.op[2].in_channels*op.op[2].out_channels)

                MAC1 = (self.width * self.height) * \
                    (op.op[1].in_channels + op.op[1].in_channels)
                MAC1 += (op.op[1].kernel_size[0]**2 * op.op[1].in_channels *
                         op.op[1].out_channels) / op.op[1].groups
                MAC2 = (self.width * self.height) * \
                    (op.op[2].in_channels + op.op[2].in_channels)
                MAC2 += (op.op[2].kernel_size[0]**2 * op.op[2].in_channels *
                         op.op[2].out_channels) / op.op[2].groups
...
class Cell(nn.Module):
    ...
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
        ...
        # _steps is set to 4, which is number of the intermediate nodes
    ...
    def forward(self, s0, s1, Z):
        ...
        offset = 0
        for i in range(self._steps):
            s = sum(self._ops[offset+j](h, Z[offset+j])[0]
                    for j, h in enumerate(states))  # Mixedop(C,stride)(x,weights)
            cost = sum(self._ops[offset+j](h, Z[offset+j])[1]
...
class Network(nn.Module):
    def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3):
        ...
        # where C equals to the initial channel that is given.
    ...
    def forward(self, input, temperature):
        ...
        for i, cell in enumerate(self.cells):  # cells는 8번 stack한 것이다.
            if cell.reduction:
                Z, score_function = self.ArchitectDist(
                    self.alphas_reduce, temperature)  # shape = [14,8]
            else:
                Z, score_function = self.ArchitectDist(
                    self.alphas_normal, temperature)
